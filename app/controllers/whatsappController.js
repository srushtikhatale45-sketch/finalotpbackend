...
const verifyWhatsAppOTP = async (req, res) => {
  try {
    ...
    const otpRecord = await OTP.findOne({
      where: { phoneNumber: cleanPhoneNumber, channel: 'whatsapp', isVerified: false, expiresAt: { [Op.gt]: new Date() } },
    ...
    if (otpRecord.otpCode === otpCode) {
      ...
      if (!user) {
        user = await User.create({ name: name || 'User', mobileNumber: cleanPhoneNumber, isVerified: true, preferredChannel: 'whatsapp' });
      } else {
      ...
      return res.status(200).json({
        success: true, verified: true, message: 'Phone number verified successfully via WhatsApp', channel: 'whatsapp',
        user: { id: user.id, name: user.name, mobileNumber: user.mobileNumber, isVerified: true, preferredChannel: user.preferredChannel }
      });
    } else {
      await otpRecord.update({ attempts: otpRecord.attempts + 1 });
      return res.status(400).json({ success: false, verified: false, message: `Invalid OTP. ${5 - (otpRecord.attempts + 1)} attempts remaining.` });
    }
  } catch (error) {
...
const resendWhatsAppOTP = async (req, res) => {
  try {
    ...
    const recentOTP = await OTP.findOne({
      where: { phoneNumber: cleanPhoneNumber, channel: 'whatsapp', createdAt: { [Op.gt]: new Date(Date.now() - 30000) } }
    });
    ...
    await OTP.destroy({ where: { phoneNumber: cleanPhoneNumber, isVerified: false, channel: 'whatsapp' } });
    await OTP.create({ phoneNumber: cleanPhoneNumber, otpCode, channel: 'whatsapp', expiresAt, attempts: 0 });
    await sendOTPviaWhatsApp(cleanPhoneNumber, otpCode);
